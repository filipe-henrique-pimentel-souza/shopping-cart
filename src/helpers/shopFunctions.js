 ...
 */
export const getIdFromProduct = (product) => product
  .querySelector('span.product__id').innerText;
...
// Requisito 10 - Somar o valor do carrinho (dividido em 3 momentos:
// 1 - Click no carrinho  2 - Adicionar item ao carrinho  3 - Onload)

export const somarTotal = () => {
  ...
  [...document.querySelectorAll('.cart__product')].forEach((item) => {
    precoTotal.innerText = (
      parseFloat(precoTotal.innerText)
      + parseFloat(item.children[1].children[1].children[0].innerText)
    ).toFixed(2);
  });
};
...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  cartButton.addEventListener('click', async () => { // Requisito 8 - Add item ao carrinho
    const itemAdd = await fetchProduct(id);
    const cadaItemCarrinho = createCartProductElement(itemAdd);
    document.querySelector('.cart__products').appendChild(cadaItemCarrinho);
    saveCartID(id);
    somarTotal(); // RQ10
  }); // Fim RQ8
